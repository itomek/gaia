...
class TestLLMClientWithLemonadeClient(unittest.TestCase):
    """Tests for LLMClient using LemonadeClient for local mode."""

    def setUp(self):
        """Set up test fixtures with mocked LemonadeClient."""
        self.patcher = patch("gaia.llm.llm_client.LemonadeClient")
        self.mock_lemonade_class = self.patcher.start()
        self.mock_client = MagicMock()
        self.mock_lemonade_class.return_value = self.mock_client

    def tearDown(self):
        """Clean up after each test."""
        self.patcher.stop()

    def test_init_creates_lemonade_client_for_local_mode(self):
        """LLMClient() creates LemonadeClient for local mode."""
        LLMClient(base_url="http://localhost:8000")
        self.mock_lemonade_class.assert_called_once()
        # Verify the call arguments
        call_kwargs = self.mock_lemonade_class.call_args[1]
        self.assertEqual(call_kwargs["host"], "localhost")
        self.assertEqual(call_kwargs["port"], 8000)
        self.assertTrue(call_kwargs["keep_alive"])

    def test_init_does_not_create_lemonade_client_for_claude_mode(self):
        """LLMClient(use_claude=True) does not create LemonadeClient."""
        with patch("gaia.llm.llm_client.AnthropicClaudeClient"):
            LLMClient(use_claude=True)
        self.mock_lemonade_class.assert_not_called()

    def test_init_does_not_create_lemonade_client_for_openai_mode(self):
        """LLMClient(use_openai=True) does not create LemonadeClient."""
        with patch("gaia.llm.llm_client.OpenAI"):
            with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
                LLMClient(use_openai=True)
        self.mock_lemonade_class.assert_not_called()

    def test_generate_completions_calls_lemonade_completions(self):
        """generate() with completions endpoint calls LemonadeClient.completions()."""
        self.mock_client.completions.return_value = {"choices": [{"text": "Hello"}]}
        client = LLMClient()
        result = client.generate("Test", endpoint="completions")
        self.mock_client.completions.assert_called_once()
        # Verify auto_download is passed
        call_kwargs = self.mock_client.completions.call_args[1]
        self.assertTrue(call_kwargs.get("auto_download"))
        self.assertEqual(result, "Hello")

    def test_generate_chat_calls_lemonade_chat_completions(self):
        """generate() with chat endpoint calls LemonadeClient.chat_completions()."""
        self.mock_client.chat_completions.return_value = {
            "choices": [{"message": {"content": "Hello from chat"}}]
        }
        client = LLMClient()
        result = client.generate("Test", endpoint="chat")
        self.mock_client.chat_completions.assert_called_once()
        # Verify auto_download is passed
        call_kwargs = self.mock_client.chat_completions.call_args[1]
        self.assertTrue(call_kwargs.get("auto_download"))
        self.assertEqual(result, "Hello from chat")

    def test_streaming_completions_returns_chunks(self):
        """Streaming with completions returns chunks correctly."""
        self.mock_client.completions.return_value = iter(
            [
                {"choices": [{"text": "Hello "}]},
                {"choices": [{"text": "world"}]},
            ]
        ...
        client = LLMClient()
        chunks = list(client.generate("Test", endpoint="completions", stream=True))
        self.assertEqual(chunks, ["Hello ", "world"])

    def test_streaming_chat_returns_chunks(self):
        """Streaming with chat returns chunks correctly."""
        self.mock_client.chat_completions.return_value = iter(
            [
        ...
        client = LLMClient()
        chunks = list(client.generate("Test", endpoint="chat", stream=True))
        self.assertEqual(chunks, ["Hello ", "world"])

    def test_auto_download_passed_to_lemonade_client(self):
        """auto_download=True is passed to LemonadeClient."""
        self.mock_client.completions.return_value = {"choices": [{"text": "OK"}]}
        client = LLMClient()
        client.generate("Test", endpoint="completions")
        call_kwargs = self.mock_client.completions.call_args[1]
        self.assertTrue(call_kwargs.get("auto_download"))

    def test_connection_error_produces_helpful_message(self):
        """Connection error produces user-friendly message."""
        from gaia.llm.lemonade_client import LemonadeClientError

        self.mock_client.completions.side_effect = LemonadeClientError(
            "Connection refused"
        )
        client = LLMClient()
    ...
    def test_get_performance_stats_uses_lemonade_client(self):
        """get_performance_stats() uses LemonadeClient.get_stats()."""
        ...
        self.assertEqual(stats["time_to_first_token"], 0.5)
        self.assertEqual(stats["tokens_per_second"], 100)
